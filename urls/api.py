...
@api.route('/admin/access_codes', methods=['GET'])
...
@api.route('/admin/access_codes', methods=['POST'])
@jwt_required(role='admin')
@jwt_get_user()
def create_access_code(admin_user):
    ...
    # Jeśli podano e-mail, wyślij powiadomienie – rozszerzamy obecną usługę mailową
    if email:
        try:
            # Zakładamy, że endpoint usługi mailowej jest dostępny pod adresem skonfigurowanym w konfiguracji
            mail_url = current_app.config.get("EMAIL_SERVICE_URL", "http://127.0.0.1:5001/send-email")
            response = requests.post(mail_url, json={"email_receiver": email, "auth_key": new_code})
...
@api.route('/admin/access_codes/<int:code_id>', methods=['DELETE'])
@jwt_required(role='admin')
